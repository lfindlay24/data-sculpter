...
def lambda_handler(event, context):
    ...
    if body["user_email"] in group["user_emails"]:
        return response(400, "User is already in the group")
    
    # send email to the user
    send_message({
        "recipient": body["user_email"],
        "subject": f"You have been added to the group {group['group_name']}",
        "message": f"You have been added to the group {group['group_name']}"
    })

    # add the user to the group
    group["user_emails"].append(body["user_email"])
    groups_table.put_item(Item=group)

    return response(200, "User added to group successfully")
